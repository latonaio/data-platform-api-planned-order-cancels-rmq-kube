...
import (
	"context"
	dpfm_api_input_reader "data-platform-api-planned-order-cancels-rmq-kube/DPFM_API_Input_Reader"
	dpfm_api_output_formatter "data-platform-api-planned-order-cancels-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-planned-order-cancels-rmq-kube/config"
...
func (c *DPFMAPICaller) headerCancel(
	...
	for i := range *items {
		...
		res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": (*items)[i], "function": "PlannedOrderItem", "runtime_session_id": sessionID})
		if err != nil {
			err = xerrors.Errorf("rmq error: %w", err)
			log.Error("%+v", err)
			return nil, nil, nil, nil
